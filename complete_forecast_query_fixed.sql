-- Complete App Review Forecast Query - Fixed for BigQuery Standard SQL
...
-- Raw submission data with filtering applied upfront
raw_submissions AS (
  SELECT 
    r.*,
    COALESCE(r.reviewers[SAFE_OFFSET(0)].reviewer_name, '') AS primary_reviewer_name
  FROM 
    `shopify-dw.apps_and_developers.app_store_app_submission_reviews_v1` r
  WHERE 
    r.submitted_at >= '2023-01-01'
    AND r.submitted_at < '2025-01-01'
),

-- Historical data aggregated by month
historical_data AS (
  SELECT 
    submission_month,
    ...
    COUNT(CASE WHEN rs.is_first_submission_review = TRUE AND rs.latest_review_state = 'SUBMITTED' THEN rs.app_store_app_submission_review_id END) AS app_submission_first,
    COUNT(CASE WHEN rs.is_first_submission_review = FALSE AND rs.latest_review_state = 'SUBMITTED' THEN rs.app_store_app_submission_review_id END) AS app_submission_re,
    COUNT(CASE WHEN rs.latest_review_state = 'SUBMITTED' THEN rs.app_store_app_submission_review_id END) AS app_submission_total,
    COUNT(DISTINCT CASE WHEN rs.latest_review_state = 'APPROVED' THEN rs.app_store_app_submission_review_id END) AS app_publish,
    ...
    -- Time to publish
    AVG(CASE WHEN rs.approved_at IS NOT NULL AND rs.submitted_at IS NOT NULL THEN DATETIME_DIFF(rs.approved_at, rs.submitted_at, DAY) END) AS avg_time_to_publish_days
    
  FROM (
    SELECT 
      rs.*,
      DATE(DATE_TRUNC(rs.submitted_at, MONTH)) AS submission_month
    FROM raw_submissions rs
  ) rs
  LEFT JOIN 
    reviewer_metadata rm ON rs.primary_reviewer_name = rm.reviewed_by
  GROUP BY 
    submission_month
),
